...
- **Live PDF Preview with True Page Breaks**: See your resume update in real-time as you type. Features a CSS multi-column layout for true, native page breaks and a built-in pagination component to navigate multi-page resumes exactly as they will print.
- **Next-Gen PDF Import & Parsing**: Instantly restore your resume data from PDFs generated by the app via embedded hidden JSON metadata (100% accurate). For other PDFs, the builder combines a high-speed local parser with an intelligent AI fallback (OpenAI) to extract your text and perfectly populate the form.
...
- **Accessible & Modern UI**: Built from the ground up using **shadcn/ui** components (including fully refactored Dialogs, Forms, and Layouts) for a visually consistent, aesthetic, and fully accessible user experience.
...
- **High-Quality Export**: Leveraging the browser's native print engine for perfect pixel-to-pixel PDF generation with fully selectable text and optimized print layouts (like border-to-border backgrounds on modern templates).
